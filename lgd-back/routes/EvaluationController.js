...
import evaluationService from "../services/EvaluationsSevice.js"
...
router.post("/addEvaluation", async function (req, res){
    ...
    const evaluation = await evaluationService.saveEvaluation(userModel);
...
router.put("/updateEvaluation/:id", async function(req, res){
    ...
    const evaluation = await evaluationService.updateEvaluationById(req.params.id, userModel);
    return res.status(200).json(evaluation);
})
